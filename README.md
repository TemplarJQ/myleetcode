...
--2019.4.1打卡--
代码还是会传的，这几天的东西转到了了印象笔记上，以后不在这里写啦。


--2019.3.21打卡--

我胡汉三又回来了！\
这几天折腾Spring全年桶搞得有些入迷，加上脚本验证的东西也看了看。\
最近写的东西应该很快会上线，尽量传到github。\
刷题不能挺啊，感觉自己退步明显·····:(\
...
今天生日，突然发现同学和自己同年同月同日生可还行····\
和鱼他们一起吃的，感觉还是很开心啊~~ \
愿望也没来得及许，过年都许完了，希望都能实现！



--2019.3.10打卡--

其实二分法一个很重要的地方在于边界判断的讨论，以及两边区间的是否有序问题，比如说像34题这个题目，就比较细致，面试遇到这种题目的时候一定要先考虑清楚，自己写几个测试用例出来分析问题。\
今天跑了趟沙河，环境真的非常适合学习，自己当年要是入学就是这么好的设施估计怨气会小很多吧，希望学弟学妹的能够收获更大更多。\
也希望北邮越来越好，已经是第五个年头啦。
...
今天看了一部分《设计模式》的初级，轮子哥说代码量不够看能就察觉不出设计模式的重要性，而例子都是很复杂的才有效果，它就是为解决复杂问题而生的，我现在的代码量就还不够大，但是觉得里面的很多东西以后肯定用得到。就当初学过一遍，以后有的可查。\
总结算法的工作还是需要专门的东西来，买了本Java的经典《算法》，估计这几天先把初始的Java基础过一次，然后分门别类地练题，总结之前自己写过的题目。\
总是再结合，也要去实践。\
知识点：继承，白箱复用（white-box reuse）；对象组合，黑箱复用。取决于内部封装是否可见。



--2019.3.4打卡--

DO急需续命，找个edu账号再来一哈算了。\
151这道题。一个java对于字符串的处理trim方法，s = s.trim()删除头尾空白符的字符串。\
二分的几道题目：\
（153、154）两道是寻找旋转数组最小值的题目，类似的还有（33、81）以及34两道都是类似题目。\
(153、154)的区别在于，由于存在重复，很可能跳过排序过程。


--2019.3.3打卡--

最近事情有点密集，倦怠了倦怠了。。。\
复习了一遍JDBC的东西，看了Python socket的源码后，我觉得有必要再回看一下TCP/UDP这套东西。\
还是文档和源码给力，一个tuple报错的问题还找了俩小时。。。\
图书馆关于java web的书都有点老了，还是花时间自己找点网上的官方文档直接开撸吧。\
LT的东西还是没有总结完，这个早上找点时间把这个任务完成了再说，有时间找个比赛打一下试试水。\
回看1.1定下来的目标，自己还差得远呢，今年的目标一点都没有做完：今天刷三道。
