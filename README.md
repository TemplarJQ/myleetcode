...
今天看了一部分《设计模式》的初级，轮子哥说代码量不够看能就察觉不出设计模式的重要性，而例子都是很复杂的才有效果，它就是为解决复杂问题而生的，我现在的代码量就还不够大，但是我觉得里面的很多东西以后肯定用得到。就当初学过一遍，以后有的可查。\
总结算法的工作还是需要专门的东西来，我买了本Java的经典《算法》，估计这几天先把初始的Java基础过一次，然后分门别类地练题，总结之前自己写过的题目。\
总是再结合，也要去实践。
